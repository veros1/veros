...
function typeWriterStart() {
  if (startIndex <= startMessage.length) {
    startTextContent = startMessage.slice(0, startIndex);
    startIndex++;
...
// blink cursor (مرة وحدة فقط)
blinkInterval = setInterval(() => {
  startCursorVisible = !startCursorVisible;

  startText.textContent =
    startTextContent + (startCursorVisible ? '|' : ' ');
}, 500);
...
typeWriterStart();
  ...
  const bioMessages = [
    "“Too many bans made me build my own lane.”",
    "\"Hello, World!\""
  ];
  let bioText = '';
  let bioIndex = 0;
  ...
  homeButton.addEventListener('click', () => {
    switchTheme('hacker_background.mp4', backgroundMusic, 'home-theme');
  });
